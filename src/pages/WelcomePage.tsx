    ...
    const removeDb = window.electronAPI.key.onDbKeyStatus((payload: { message: string; level: number }) => {
      ...
      if (isDbKeyReadyMessage(normalizedMessage)) {
        window.electronAPI.notification?.show({
          title: 'OmniMindWeChat 准备就绪',
          content: '现在可以登录微信了',
          avatarUrl: './logo.png',
          sessionId: 'omnimind-wechat-system'
        })
      }
    })
        ...
        <div className="welcome-container">
          ...
          <div className="welcome-sidebar">
            <div className="sidebar-header">
              <img src="./logo.png" alt="OmniMindWeChat" className="sidebar-logo" />
              <div className="sidebar-brand">
                <span className="brand-name">OmniMindWeChat</span>
    ...
    <div className={rootClassName}>
      <div className="welcome-container">
        ...
        <div className="welcome-content">
          ...
          {currentStep.id === 'intro' && (
            <div className="intro-footer">
              ...
              <p>OmniMindWeChat 需要访问你的本地数据文件以提供分析与导出功能。</p>
            </div>
          )}
        ...
        <ConfirmDialog
            ...
            title="开始获取数据库密钥"
            message={`当开始获取后 OmniMindWeChat 将会执行准备操作。
${isLinux ? `
【⚠️ Linux 用户特别注意】
如果您在微信里勾选了“自动登录”，请务必先关闭自动登录，然后再点击下方确认！
（因为授权弹窗输入密码需要时间，若自动登录太快会导致获取失败）
` : ''}
当 OmniMindWeChat 内的提示条变为绿色显示允许登录或看到来自 OmniMindWeChat 的登录通知时，请在手机上确认登录微信。`}
